...
The [Lucide](https://lucide.dev) icon set as Vide components for roblox-ts,
precompiled through [`@rbxts/svg`](../svg).

```bash
npm install @rbxts/lucide-vide @rbxts/svg @rbxts/svg-vide
```
...
Everything [`<Svg>`](../svg-vide) takes, except `source` — the icon already is
one.
...
Under Vide every one of them may also be a source, and reactivity is
`@rbxts/svg-vide`'s rather than this package's — the wrapper adds no effect and
no state of its own:
...
Changing `colour` writes one `ImageColor3` and rasterizes nothing.
